...
bw_files = [
    'ENCFF622SLI.bigWig',  # rep1
    'ENCFF384EPM.bigWig',  # rep2
    'ENCFF382ARH.bigWig'   # rep3
]
...
RGB_COLOR_MAP = {
    0: '106,13,173', 1: '0,0,255', 2: '0,255,0', 3: '255,255,0', # purple, blue, green, yellow
    4: '255,165,0', 5: '255,0,0', 6: '153,0,0' # ornage, red, crimson
}
...
def hmm_states_to_bed(states, chrom, resolution, filename, color_map):
    ...
    with open(filename, 'w') as f:
        ...
        for i, state in enumerate(states):
            ...
            color = color_map.get(int(state), '255,255,255') # default to white if state not in map
...
def estimate_hmm_params(x, y, n_states, eps=1e-8):
    ...
    Output:
      means, variances, priors, transition matrix, logL 
    """
...
def save_model_parameters(means, variances, pi, A, logL, seq_len, out_path):
    """
    ...
    with open(out_path, "w", newline="") as f:
        ...
        w.writerow(["LogL", f"{logL:.4f}"])
        w.writerow(["AIC", f"{aic:.4f}"])
        w.writerow(["BIC", f"{bic:.4f}"])
        w.writerow([])

        # emissions
        w.writerow(["Section", "State Index", "Mean", "Variance"])
        for i in range(N):
...
def process_replicate(bw_file):
    ...
    for n_states in range(2, 8):
        ...
        # empirically est parameters from decoded path
        ...
        bed_path = os.path.join(
            output_dir, f"{cl}.{chrom}.{replicate_name}.{n_states}states.bed"
        )
        hmm_states_to_bed(y_sorted, chrom, resolution, bed_path, RGB_COLOR_MAP)
        print(f"Wrote {bed_path}", flush=True)

        # save empirical param file
        param_path = os.path.join(
            output_dir, f"{cl}.{chrom}.{replicate_name}.{n_states}states.params.tsv"
        )
        save_model_parameters(means_sorted, vars_sorted, pi_sorted, A_sorted, logL_emp, seq_len, param_path)
